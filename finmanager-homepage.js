...
async function populateTableWithReimTickets() {
    ...
    for (let i = 0; i < reimticketArray.length; i++) {
        ...
        let td1 = document.createElement('td'); // Updated ReimTicketDAO (line 29) now retrieves reimauthorId correctly.
        td1.innerHTML = reimticket.reimauthorId; 

        let td2 = document.createElement('td');
        td2.innerHTML = reimticket.reimticketType;

        let td3 = document.createElement('td'); // Updated ReimTicketDAO (line 34) now retrieves reimticketId correctly.
        td3.innerHTML = reimticket.reimticketId;
     ...
     // If the reimticket has already been reimubursed (already has reimamount), display the reimamount and resolverId
        if (reimticket.resolverId != 0) {
            ...
            reimamountButton.innerText = 'Update Status';
